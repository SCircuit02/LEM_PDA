 ...
 * Nuevo en 0.1.3:
 *   - SDI-12: vista de datos con SCROLL que muestra TODOS los valores del
 *     sensor (lee aD0!..aD9!, usa aC! para >9 valores tipo AquaCheck);
 *     formato "- valor" por linea (sin V0:/V1:).
 * Nuevo en 0.1.2:
...
// Usa medicion concurrente aC! (cuenta de hasta 2 digitos -> mas de 9 valores,
// p.ej. AquaCheck); si no responde, cae a aM! (1 digito).
void sdi12DoMeasure(char addr) {
  sdi12IdStr     = "";
  sdi12NumValues = 0;
  ...
  // Medicion: aC! concurrente "atttnn" (nn hasta 2 digitos); fallback aM! "atttn"
  int waitSec = 0, expected = 0;
  String mr = sdi12Command(String(addr) + "C!", 400);
  if (mr.length() >= 6 && mr.charAt(0) == addr) {
    waitSec  = mr.substring(1, 4).toInt();
    expected = mr.substring(4).toInt();
  } else {
    mr = sdi12Command(String(addr) + "M!", 400);
    if (mr.length() >= 5 && mr.charAt(0) == addr) {
      waitSec  = mr.substring(1, 4).toInt();
  ...
  // Leer aD0!..aD9! acumulando hasta 'expected' o hasta que no haya mas datos
  for (int d = 0; d <= 9; d++) {
    ...
    if (added == 0) break;
    if (expected > 0 && sdi12NumValues >= expected) break;
