// index_html.h - Pagina web del QC LemBot 0.2.2 (la incluye esp-s3-v0.2.2.ino).
...
<div class="box"><h2>Monitor UART (GP18, 115200)</h2><pre id="sUart"></pre></div>
</section>
...
<div id="sdiDev" class="ban warn">El equipo está usando el bus SDI-12 desde su pantalla. Espere a que termine.</div>
...
<p class="mut">Mientras corre una prueba no se puede iniciar otra. Durante la descarga de batería el monitor de corriente queda en pausa.</p>
</section>
...
function rData(){const c=D.cur;
 $('st').textContent='Bat '+D.bat.p+' % · '+f(D.bat.v,2)+' V';$('st').className='pill';$('apn').textContent=D.ap;
 ban('err',D.err,'bad');
...
function rSen(){const e=D.env,b=D.bat;
 ...
 $('sAds').innerHTML=D.ads?tbl(D.ads.map((v,k)=>['A'+k,f(v,4)+' V'])):'<p class="mut">ADS1115 no detectado (A0 y A1 se usan en Sense-QC).</p>';
 const hl=v=>v?'ALTO':'BAJO';$('sGpio').innerHTML=tbl([['GP2 · Pulse1',hl(D.p1)],['GP4 · Pulse2',hl(D.p2)],['GP18 · UART RX',hl(D.urx)]]);
 const so=D.soil;
...
// ---------- tests ----------
function rTst(){const a=D.sa,w=D.wx,d=D.dis,busy=a.st==='test'||w.st==='test'||d.st==='run',no=!!D.err;let h;
 ...
 $('tSa').innerHTML=h;$('bSa').disabled=busy||no;$('bSa').textContent=a.st==='res'?'Repetir Sense-QC':'Iniciar Sense-QC';
 ...
 $('tWx').innerHTML=h;$('bWx').disabled=busy||no;$('bWx').textContent=w.st==='res'?'Repetir Weather-QC':'Iniciar Weather-QC';
...
// ---------- equipo ----------
function rSys(){$('sys').innerHTML=tbl([['Firmware','QC LemBot v'+esc(D.fw)+' (ESP32-S3)'],['Red WiFi',esc(D.ap)],['Dirección','http://192.168.4.1'],
