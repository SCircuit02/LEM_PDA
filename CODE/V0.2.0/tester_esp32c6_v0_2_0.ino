/*
 * QC LemBot - ESP32-C6 Super Mini
 * Version: 0.2.0
 * Port a ESP32-C6 (RISC-V, WiFi 6 + BT 5.3 + 802.15.4). Requiere el core
 * arduino-esp32 v3.x ("Esp32c6 Dev" en el IDE).
 *
 * Mapa de pines C6 (pines "seguros" sin bootstrapping para UI y SDI-12,
 * segun pinout del C6 Super Mini):
 *   Siempre activos (pines seguros):
 *     GP0=EncA GP1=EncB GP2=Back GP3=Confirm GP14=BtnEnc GP22=SDA GP23=SCL
 *     GP21=SDI-12 TX  GP20=SDI-12 RX (dedicados, ya no compartidos)
 *   Modulos (pines con precaucion, solo se usan dentro del modulo):
 *     GP6=Pulse1 GP7=Pulse2 (JTAG; sin conflicto si no se depura por JTAG)
 *     GP4=SHT10 data GP5=SHT10 clk
 *     GP15=UART monitor RX (JTAG_SEL; el LED azul onboard parpadea con datos)
 *   Reservados: GP8=RGB WS2812 (debug visual) GP9=BOOT GP12/13=USB GP18/19=flash
 *
 * Nuevo en 0.2.0:
 *   - LED RGB (WS2812 en GP8) como debug visual de estados.
 *   - Menu "Zigbee" (work in progress; el C6 tiene radio 802.15.4).
...
// ============================================
// UI e I2C en pines SEGUROS (sin bootstrapping): 0,1,2,3,14,20,21,22,23
#define ENCODER_TRA 0    // seguro
#define ENCODER_TRB 1    // seguro
#define BTN_BACK    2    // seguro
#define BTN_CONFIRM 3    // seguro
#define BTN_ENCODER 14   // seguro
#define SDA_PIN     22   // seguro
#define SCL_PIN     23   // seguro
// Entradas de modulos en pines con precaucion (solo JTAG, no strapping):
#define PULSE1_PIN  6    // JTAG MTCK - ok si no se depura por JTAG
#define PULSE2_PIN  7    // JTAG MTDO - ok si no se depura por JTAG
#define UART_RX_PIN 15   // JTAG_SEL; LED azul onboard parpadea con los datos

#define SHT10_DATA_PIN  4   // MTMS - solo activo dentro de Weather-QC
#define SHT10_CLOCK_PIN 5   // MTDI - solo activo dentro de Weather-QC

// LED RGB WS2812 onboard (debug visual). GP8 es strapping: SOLO se usa
// para el LED, nunca como GPIO generico.
#define RGB_LED_PIN     8

// ============================================
// SDI-12 FRONT-END DE HARDWARE (2 pines DEDICADOS y seguros)
// ============================================
// TX = GP21 -> SN74LVC1G3157 -> SN74LVC1G240 (inversor) -> bus SDI-12
// RX = GP20 <- SN74AHC1G14 (inversor Schmitt) <- bus SDI-12
...
#define SDI12_TX_PIN    21  // seguro, dedicado
#define SDI12_RX_PIN    20  // seguro, dedicado
...
String sdi12IdStr       = "";      // identificacion (aI!)
...
void setup() {
  ...
  u8g2.setFont(u8g2_font_6x10_tf);
  u8g2.drawStr(14, 46, "v0.2.0 ESP32-C6");
  u8g2.sendBuffer();
...
void handleEncoder() {
  ...
  if (currentState == MENU_MAIN) {
    ...
    // Scroll por todos los valores del sensor
    if (sdi12NumValues > SDI12_DATA_VISIBLE) {
      sdi12DataScroll += diff;
      if (sdi12DataScroll < 0) sdi12DataScroll = 0;
      int maxOff = sdi12NumValues - SDI12_DATA_VISIBLE;
...
// ============================================
// LED RGB (WS2812 en GP8) - DEBUG VISUAL
// ============================================
...
void drawCurrentView() {
  ...
  if (currentViewPage == 0) {
    // Pagina 1: corriente actual / mediana / minima>0 / maxima
    drawCurRow(Y1, true,                       "%.3f", currentCurrent, "mA");
    drawCurRow(Y2, statsOk,                     "%.2f", median,         "med");
    drawCurRow(Y3, (min_current_mA > 0.0f),     "%.3f", min_current_mA, "min");
    drawCurRow(Y4, (max_current_mA > 0.0f),     "%.2f", max_current_mA, "max");
  } else {
...
void drawRawDataView() {
  ...
  switch (rawDataPage) {
    ...
    case 2: {
      ...
      sprintf(buf, "UART_RX(15):    %s", digitalRead(UART_RX_PIN)?"HIGH":"LOW"); u8g2.drawStr(2, 45, buf);
...
void drawZigbeeView() {
  ...
  u8g2.setFont(u8g2_font_4x6_tf);
  u8g2.drawStr(70, 32, "radio 802.15.4");
  u8g2.drawStr(70, 40, "del ESP32-C6");
...
void drawSDI12View() {
  ...
  if (sdi12State == SDI12_IDLE) {
    ...
    u8g2.drawStr(5, 40, "Bus: TX=GP21 RX=GP20");
    u8g2.setFont(u8g2_font_4x6_tf);
  ...
  if (sdi12State == SDI12_SENSOR) {
    ...
    // Cada valor en su linea: "- <valor>"
    u8g2.setFont(u8g2_font_5x7_tf);
    for (int i = 0; i < SDI12_DATA_VISIBLE && (i + sdi12DataScroll) < sdi12NumValues; i++) {
      int idx = i + sdi12DataScroll;
      sprintf(buf, "- %.2f", sdi12Values[idx]);
      u8g2.drawStr(4, 20 + i * 9, buf);   // 20,29,38,47,56
    }
    drawScrollBar(122, 12, 46, sdi12DataScroll, sdi12NumValues);

    // Pie: rango visible/total + atajos (OK re-pide aC!, click encoder aM!)
    u8g2.setFont(u8g2_font_4x6_tf);
    int first = sdi12DataScroll + 1;
    int last  = sdi12DataScroll + SDI12_DATA_VISIBLE;
    if (last > sdi12NumValues) last = sdi12NumValues;
    sprintf(buf, "%d-%d/%d OK:aC Enc:aM BK", first, last, sdi12NumValues);
...
// -------- PINOUT (ESP32-C6, 3 paginas) --------
void drawPinoutView() {
  char buf[20];
  u8g2.setFont(u8g2_font_6x10_tf);
  sprintf(buf, "Pinout C6 [%d/3]", pinoutPage + 1);
  u8g2.drawStr(15, 10, buf);
  u8g2.drawLine(0, 12, 128, 12);
  u8g2.setFont(u8g2_font_5x7_tf);

  if (pinoutPage == 0) {          // UI (pines seguros)
    ...
    u8g2.drawStr(2, 58, "GP14 - BTN Encoder");
  } else if (pinoutPage == 1) {   // Comunicaciones (pines seguros)
    u8g2.drawStr(2, 22, "GP22 - SDA I2C");
    u8g2.drawStr(2, 31, "GP23 - SCL I2C");
    u8g2.drawStr(2, 40, "GP21 - SDI-12 TX");
    u8g2.drawStr(2, 49, "GP20 - SDI-12 RX");
    u8g2.drawStr(2, 58, "GP15 - UART RX (LED az)");
  } else {                        // Testers y LED
    u8g2.drawStr(2, 22, "GP6  - Pulse1");
    u8g2.drawStr(2, 31, "GP7  - Pulse2");
    u8g2.drawStr(2, 40, "GP4  - SHT10 Data");
    u8g2.drawStr(2, 49, "GP5  - SHT10 Clk");
    u8g2.drawStr(2, 58, "GP8  - RGB WS2812");
  }
