 ...
 * Nuevo en 0.1.2:
 *   - SDI-12 con front-end de hardware (proteccion de sensores):
 *       TX = GPIO7  -> SN74LVC1G3157 (switch) -> SN74LVC1G240 (buffer inversor) -> bus
 *       RX = GPIO6  <- SN74AHC1G14 (inversor Schmitt) <- bus
 *     Las dos inversiones cancelan la del bus SDI-12: el MCU ve UART normal
 *     1200 baud 7E1 NO invertido. Se reemplaza la libreria de 1 pin por un
 *     bit-bang propio de 2 pines (TX/RX).
 */
    ...
    switch (sdi12State) {
      ...
      case SDI12_LIST:
        if (sdi12Sel < sdi12Count) {
          // Sensor seleccionado -> ver sus datos
      ...
      case SDI12_CHG_SELSRC:
        ...
        break;
      case SDI12_SENSOR:
        sdi12NeedMeasure = true;   // re-medir
        break;
      case SDI12_NEWID:
...
void sdi12SendChar(uint8_t ch) {
  ...
  // bit de paridad par (1s totales pares)
...
void sdi12SendBreakAndCommand(const String& cmd) {
  pinMode(SDI12_TX_PIN, OUTPUT);
  // break: espacio continuo
  digitalWrite(SDI12_TX_PIN, SDI12_TX_SPACE);
  delay(13);
  // marca de arranque
  digitalWrite(SDI12_TX_PIN, SDI12_TX_MARK);
  delay(9);
  // caracteres del comando (incluye el '!')
  for (int i = 0; i < (int)cmd.length(); i++) sdi12SendChar((uint8_t)cmd.charAt(i));
  // volver a reposo (marca) -> libera el bus para la respuesta del sensor
  digitalWrite(SDI12_TX_PIN, SDI12_TX_MARK);
}
...
int sdi12ReadChar(unsigned long timeoutMs) {
  ...
  // esperar el start bit: la linea en reposo esta en marca; el start la baja a espacio
  while (digitalRead(SDI12_RX_PIN) == SDI12_RX_MARK) {
    if (millis() - t0 >= timeoutMs) return -1;
  }
  noInterrupts();
  // ir al centro del primer bit de datos (1.5 bits desde el flanco de start)
  delayMicroseconds(SDI12_BIT_US + SDI12_HALF_US);
  uint8_t c = 0;
  for (int i = 0; i < 7; i++) {
    if (digitalRead(SDI12_RX_PIN) == SDI12_RX_MARK) c |= (1 << i);  // marca = 1
    delayMicroseconds(SDI12_BIT_US);
  }
  // saltar el bit de paridad para terminar dentro del stop (marca) y no
  // confundir su nivel con el start del siguiente caracter
  delayMicroseconds(SDI12_BIT_US);
  interrupts();
  return c & 0x7F;
}
...
// Misma firma que antes: el resto del codigo no cambia.
String sdi12Command(const String& cmd, uint16_t waitMs) {
...
// Mide un sensor: identificacion (aI!) + medicion (aM!/aD0!)
void sdi12DoMeasure(char addr) {
  sdi12IdStr     = "";
  sdi12NumValues = 0;
  ...
  if (idr.length() > 1) sdi12IdStr = idr.substring(1); // quitar la direccion inicial

  // Iniciar medicion: respuesta "atttn" (ttt seg, n valores)
  String mr = sdi12Command(String(addr) + "M!", 400);
  int waitSec  = 0;
  int expected = 0;
  if (mr.length() >= 5) {
    waitSec  = mr.substring(1, 4).toInt();
    expected = mr.substring(4, 5).toInt();
  }

  // Esperar a que el sensor complete la medicion (tiempo indicado, acotado)
  unsigned long limit = (unsigned long)waitSec * 1000UL;
  if (limit > 9000UL) limit = 9000UL;   // tope de seguridad
  delay(limit + 150);

  // Pedir los datos
  String dr = sdi12Command(String(addr) + "D0!", 600);

  // Parsear valores: "a+1.23-4.56+7.8..." separados por + o -
  if (dr.length() > 1) {
    String body = dr.substring(1); // quitar direccion
    String num = "";
    for (int i = 0; i < (int)body.length(); i++) {
      char c = body.charAt(i);
      if ((c == '+' || c == '-') && num.length() > 0) {
        if (sdi12NumValues < SDI12_MAX_VALUES) sdi12Values[sdi12NumValues++] = num.toFloat();
        num = "";
        num += c;
      } else {
        num += c;
    ...
    if (num.length() > 0 && sdi12NumValues < SDI12_MAX_VALUES) {
      sdi12Values[sdi12NumValues++] = num.toFloat();
    }
  }
  (void)expected;
}
...
void updateSDI12() {
  switch (sdi12State) {
    ...
    case SDI12_SENSOR:
      if (sdi12NeedMeasure) {
        sdi12ShowBusy("Midiendo...");
        sdi12DoMeasure(sdi12CurAddr);
...
void drawSDI12View() {
  ...
  if (sdi12State == SDI12_SENSOR) {
    u8g2.setFont(u8g2_font_5x7_tf);
    sprintf(buf, "Sensor %c", sdi12CurAddr);
    u8g2.drawStr(2, 20, buf);
    // Identificacion (truncada al ancho de pantalla)
    if (sdi12IdStr.length() > 0) {
      u8g2.setFont(u8g2_font_4x6_tf);
      u8g2.drawStr(2, 30, sdi12IdStr.substring(0, 31).c_str());
    }
    // Valores (hasta 6 en 2 columnas x 3 filas)
    u8g2.setFont(u8g2_font_5x7_tf);
    if (sdi12NumValues == 0) {
      u8g2.drawStr(2, 44, "Sin datos");
    } else {
      int shown = sdi12NumValues > 6 ? 6 : sdi12NumValues;
      for (int i = 0; i < shown; i++) {
        int col = (i < 3) ? 2 : 68;
        int row = 40 + (i % 3) * 8;   // 40, 48, 56
        sprintf(buf, "V%d:%.2f", i, sdi12Values[i]);
        u8g2.drawStr(col, row, buf);
      }
      if (sdi12NumValues > 6) {
        u8g2.setFont(u8g2_font_4x6_tf);
        sprintf(buf, "+%d", sdi12NumValues - 6);
        u8g2.drawStr(116, 40, buf);
      }
    }
    u8g2.setFont(u8g2_font_4x6_tf);
    u8g2.drawStr(0, 63, "OK:Remedir  BACK:Lista");
    return;
  }
